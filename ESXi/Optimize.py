...
def get_user_inputs():
    ...
    """
    # Define min and max allowed values for each parameter
    limits = {
    ...
    # Loop through each parameter and validate user input
    for param, limit in limits.items():
        while True:
            ...
            # Use default max value if no input
            if not user_input:
...
def execute_command(command):
    ...
    try:
        ...
        if result.returncode != 0:
            # Log error output if the command fails
...
def optimize_esxi():
    ...
    # Get user inputs for optimization parameters
    ...
    execute_command((
        ...
        f'max_queues={user_inputs["max_queues"]} '
        f'RSS={user_inputs["RSS"]} '
        f'DYN_RSS={user_inputs["DYN_RSS"]} '
        f'DRSS={user_inputs["DRSS"]}"'
    ))
    # Verify nmlx5_core module parameters
